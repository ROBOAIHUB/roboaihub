...
class SheetManager:
    ...
    def create_month_sheets_for_employee(self, drive_manager, emp_name, folder_id, month, year):
        """
        Generates daily sheets for a specific employee.
        ...
        for day in range(1, num_days + 1):
            ...
            try:
                ...
                if not files:
                    # Create Sheet
                    drive_manager.create_spreadsheet(day_str, month_folder_id)
    ...
    def create_month_sheets_for_all(self, drive_manager, user_manager, month, year):
        ...
        total_summary = {"success": 0, "skipped": 0, "errors": []}
        
        # Get number of days in month (just for logging context)
        _, num_days = calendar.monthrange(year, month)
        month_str = datetime.date(year, month, 1).strftime("%B_%Y")
        ...
        for emp_id, emp_data in employees.items():
            if emp_id == 'RAH-000': continue # Skip Master Admin
            
            emp_name = emp_data.get('name')
            folder_id = emp_data.get('folder_id')
            
            # Use the single employee method
            emp_summary = self.create_month_sheets_for_employee(drive_manager, emp_name, folder_id, month, year)
            
            # Merge results
            total_summary["success"] += emp_summary["success"]
            total_summary["skipped"] += emp_summary["skipped"]
            total_summary["errors"].extend(emp_summary["errors"])
