        ...
        <div className="relative p-6">
            ...
            <div className="mt-8 pt-6 border-t border-yellow-500/30">
                <h4 className="text-lg text-neon-blue mb-4 font-bold flex items-center gap-2">
                    <span className="text-xl">📢</span> GLOBAL BROADCAST
                </h4>
                <div className="flex gap-4">
                    <input
                        ...
                        className="flex-1 bg-space-900 border border-blue-800 text-white p-3 rounded focus:border-neon-blue focus:outline-none"
                    />
                    <button
                        onClick={async () => {
                            const msgText = document.getElementById('broadcastMsg').value;
                            if (!msgText) return alert("Please enter a message");
                            if (!window.confirm("Send this message to ALL employees?")) return;

                            setMsg('Broadcasting... 📡');
                            try {
                                const res = await api.post('/admin/notifications', { message: msgText, target_emp_id: 'all' });
                                setMsg(`Broadcast Sent! ${res.data.message} ✅`);
                                document.getElementById('broadcastMsg').value = '';
                            } catch (err) {
                                console.error(err);
                                setMsg('Broadcast Failed. ❌');
                            }
                        }}
                        className="px-6 py-3 bg-neon-blue text-black font-bold rounded hover:bg-white transition uppercase tracking-wider"
                    >
                        SEND
                    </button>
                </div>
